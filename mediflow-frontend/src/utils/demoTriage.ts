...
// (mediflow-backend/src/controllers/aiController.ts). Used only by the
// landing-page hero teaser, which is deliberately zero-network for an
// instant first touch — the /demo page itself calls the real backend.
...
// when OpenAI is configured. Genuinely useful on its own.
export const SELF_CARE_GUIDANCE: Record<Urgency, string> = {
...
const RULES: { keywords: string[]; specialization: string; urgency: Urgency }[] = [
  ...
  { keywords: ['headache', 'migraine', 'dizziness', 'numbness'], specialization: 'Neurology', urgency: 'medium' },
...
export function demoTriage(answers: TriageAnswers): DemoTriageResult {
  ...
  return {
    ...
    urgency,
    reasoning,
    selfCare: SELF_CARE_GUIDANCE[urgency],
  };
}
